...
import _graphql, { DOMException } from './graphql'

function sendGqlMonitor(
  url: string,
...
export const link = createHttpLink({
  ...
  fetch(input: string /* RequestInfo */, init?: RequestInit | undefined) {
    return _graphql(input, init).catch(function (error) {
      sendGqlMonitor(input, init, error)
      if (/abort/.test(JSON.stringify(error))) {
